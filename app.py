...
def update1(id):
    ...
    if request.method == 'POST':
        try:
            cursor1 = db.execute('select id, tenant_name1 from tenant1 where id = ?', [id])
            tenant_to_update = cursor1.fetchone()
            name1 = request.form['tenant_1']
            db.execute('update tenant1 set tenant_name1 = ? where id = ?', [name1, id])
            db.commit()
            return redirect(url_for('update'))
        except:

            cursor2 = db.execute('select id, tenant_name2 from tenant2 where id = ?', [id])
            tenant_to_update = cursor2.fetchone()
            name2 = request.form['tenant_2']
            db.execute('update tenant2 set tenant_name2 = ? where id = ?', [name2, id])
            db.commit()
            return redirect(url_for('update'))
...
def update2(id):
    ...
    if request.method == 'POST':
        try:
            cursor3 = db.execute('select id, tenant_name3 from tenant3 where id = ?', [id])
            tenant_to_update = cursor3.fetchone()
            name3 = request.form['tenant_3']
            db.execute('update tenant3 set tenant_name3 = ? where id = ?', [name3, id])
            db.commit()
            return redirect(url_for('update'))
        except:

            cursor4 = db.execute('select id, tenant_name4 from tenant4 where id = ?', [id])
            tenant_to_update = cursor4.fetchone()
            name4 = request.form['tenant_4']
            db.execute('update tenant4 set tenant_name4 = ? where id = ?', [name4, id])
            db.commit()
            return redirect(url_for('update'))
    return render_template('update.html')
...
def update3(id):
    ...
    db = get_db()#initialise the database
    if request.method == 'POST':
            cursor5 = db.execute('select id, tenant_rubbish from rubbish_removal where id = ?', [id])
            tenant_to_update = cursor5.fetchall()
